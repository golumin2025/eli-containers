    ...
    handler: async (input) => {
      if (import.meta.env.DEV === true) {
        try {
          ...
          const successUrl = `https://app.miboxmovingandstorage.com/?type=${input.serviceType}&email=${input.email}&new_zipcode=${input.finalDeliveryZip}&phone_number=${input.phone}&start_date=${input.deliveryDate}&zipcode=${input.initialDeliveryZip}&promocode=promo`;
      ...
      // Handle non-DEV environment or add a default return
      return {
        success: false,
        message: "Form submission only available in development mode",
      };
    },
  }),
};
